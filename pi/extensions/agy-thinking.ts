...
export default function (pi: ExtensionAPI) {
  ...
  let chars = 0;
  // Tool-output expansion state, read natively from ctx.ui (pi >= 0.85 exposes
  // getToolsExpanded(); no need to monkey-patch InteractiveMode via a fragile
  // jiti import that breaks whenever pi's dist graph pulls in pi-server).
  let uiCtx: any = null;
  function isExpanded(): boolean {
    try {
      return !!uiCtx?.ui?.getToolsExpanded?.();
    } catch {
      return false;
    }
  }
