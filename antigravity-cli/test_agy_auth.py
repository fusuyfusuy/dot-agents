...
class TestAgyAuth(unittest.TestCase):
    def setUp(self):
        ...
        self.patchers = [
            ...
            patch.object(agy_auth, "ACTIVE_ENV_FILE", self.agy_cli_dir / "active_env.sh"),
            patch.object(agy_auth, "ACTIVE_ENV_JSON", self.agy_cli_dir / "active_env.json"),
    ...
    def test_set_project_and_env_export(self):
        """Verify linking a GCP project updates profile.json, active_env.sh and status JSON."""
        agy_auth.cmd_save("user1")
        self._seed_active_credentials("user2@work.com", "User Two", "token-user2-secret")
        agy_auth.cmd_save("user2")

        # Link project to user1
        ret = agy_auth.cmd_set_project("gcp-proj-user1", target="user1")
        self.assertEqual(ret, 0)

        # Link project to active user2
        ret = agy_auth.cmd_set_project("gcp-proj-user2")
        self.assertEqual(ret, 0)

        # Verify active_env.sh holds user2's project
        env_content = (self.agy_cli_dir / "active_env.sh").read_text(encoding="utf-8")
        self.assertIn('export GOOGLE_CLOUD_PROJECT="gcp-proj-user2"', env_content)
        self.assertIn('export AGY_ACTIVE_PROFILE="user2"', env_content)

        # Switch to user1 -> active_env.sh must now hold user1's project
        agy_auth.cmd_switch("user1")
        env_content = (self.agy_cli_dir / "active_env.sh").read_text(encoding="utf-8")
        self.assertIn('export GOOGLE_CLOUD_PROJECT="gcp-proj-user1"', env_content)
        self.assertIn('export AGY_ACTIVE_PROFILE="user1"', env_content)

        # Check JSON status includes gcp_project
        import io
        from contextlib import redirect_stdout
        f = io.StringIO()
        with redirect_stdout(f):
            agy_auth.cmd_status(json_output=True)
        status_data = json.loads(f.getvalue())
        self.assertEqual(status_data["active_gcp_project"], "gcp-proj-user1")

        prof_projects = {p["name"]: p["gcp_project"] for p in status_data["profiles"]}
        self.assertEqual(prof_projects["user1"], "gcp-proj-user1")
        self.assertEqual(prof_projects["user2"], "gcp-proj-user2")

        # Unlink project
        ret = agy_auth.cmd_set_project("none", target="user1")
        self.assertEqual(ret, 0)
        env_content = (self.agy_cli_dir / "active_env.sh").read_text(encoding="utf-8")
        self.assertIn("unset GOOGLE_CLOUD_PROJECT", env_content)
