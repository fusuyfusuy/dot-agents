...
ACTIVE_ENV_FILE = AGY_CLI_DIR / "active_env.sh"
ACTIVE_ENV_JSON = AGY_CLI_DIR / "active_env.json"
    ...
    if profile_meta_path and profile_meta_path.is_file():
        try:
            with open(profile_meta_path, "r", encoding="utf-8") as f:
                ...
                if cached_meta.get("gcp_project"):
                    info["gcp_project"] = cached_meta.get("gcp_project")
        except Exception:
            pass
...
def list_profiles() -> List[str]:
    ...
    priority = {"yusufakcakaya": 1, "davaytiyada": 2}
    return sorted(profiles, key=lambda p: (priority.get(p.lower(), 99), p.lower()))


def sync_active_env(
    curr_profile: Optional[str] = None,
    email: Optional[str] = None,
    gcp_project: Optional[str] = None,
) -> None:
    """Write active environment variables for shell export and launchers."""
    curr = curr_profile or get_current_profile_name() or ""
    proj = gcp_project
    if proj is None and curr:
        meta_file = PROFILES_DIR / curr / "profile.json"
        if meta_file.is_file():
            try:
                with open(meta_file, "r", encoding="utf-8") as f:
                    meta = json.load(f)
                    proj = meta.get("gcp_project")
            except Exception:
                pass

    clean_email = email or ""
    env_lines = [
        "# Antigravity Active Environment - Auto-generated by agy-auth",
        f'export AGY_ACTIVE_PROFILE="{curr}"',
        f'export AGY_ACTIVE_EMAIL="{clean_email}"',
    ]
    if proj:
        env_lines.extend([
            f'export GOOGLE_CLOUD_PROJECT="{proj}"',
            f'export CLOUDSDK_CORE_PROJECT="{proj}"',
            f'export AGY_PROJECT="{proj}"',
        ])
    else:
        env_lines.extend([
    ...
    try:
        ...
        set_secure_permissions(ACTIVE_ENV_FILE)

        env_data = {
            "active_profile": curr,
            "email": email or "",
            "gcp_project": proj or None,
            "updated_at": datetime.now(timezone.utc).isoformat(),
        }
        ACTIVE_ENV_JSON.write_text(json.dumps(env_data, indent=2), encoding="utf-8")
        set_secure_permissions(ACTIVE_ENV_JSON)
    except Exception:
        pass
...
def sync_active_to_current_profile() -> None:
    ...
    info = extract_account_info(
        ...
        profile_meta_path=prof_dir / "profile.json",
    )
    meta = {}
    if meta_file.is_file():
        try:
    ...
    sync_active_env(curr, info.get("email"), meta.get("gcp_project"))
...
def cmd_save(name: Optional[str] = None, email_hint: Optional[str] = None) -> int:
    ...
    if existing_meta.get("gcp_project"):
        meta["gcp_project"] = existing_meta.get("gcp_project")

    meta_file.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    set_secure_permissions(meta_file)

    set_current_profile_name(name)
    sync_active_env(name, email, meta.get("gcp_project"))
    print(f"{GREEN}✓ Saved profile:{RESET} {BOLD}{name}{RESET} ({email})")
    return 0


def cmd_set_project(project_id: str, target: Optional[str] = None) -> int:
    """Link or unlink a Google Cloud Project ID (with Developer/Cloud Credits) to a profile."""
    profiles = list_profiles()
    if not profiles:
        print(f"{RED}Error:{RESET} No profiles found. Run `agy-auth save <name>` first.")
        return 1

    matched_profile = None
    if not target:
        curr = get_current_profile_name()
        matched_profile = curr or profiles[0]
    elif target.isdigit():
        idx = int(target) - 1
    ...
    if clean_proj.lower() in ("none", "clear", "unset", "remove", ""):
        meta.pop("gcp_project", None)
        meta_file.write_text(json.dumps(meta, indent=2), encoding="utf-8")
        set_secure_permissions(meta_file)
        print(f"{YELLOW}✓ Unlinked GCP Project from profile:{RESET} {BOLD}{matched_profile}{RESET}")
        curr = get_current_profile_name()
        if curr == matched_profile:
            sync_active_env(curr, meta.get("email"), None)
        return 0

    meta["gcp_project"] = clean_proj
    meta_file.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    set_secure_permissions(meta_file)
    print(f"{GREEN}✓ Linked GCP Project:{RESET} {BOLD}{clean_proj}{RESET} → profile {BOLD}{matched_profile}{RESET}")

    curr = get_current_profile_name()
    if curr == matched_profile:
        sync_active_env(curr, meta.get("email"), clean_proj)
    return 0
...
def cmd_switch(target: str, quiet: bool = False) -> int:
    ...
    gcp_proj = info.get("gcp_project")
    sync_active_env(matched_profile, info.get("email"), gcp_proj)

    if not quiet:
        proj_str = f" | Project: {PURPLE}{gcp_proj}{RESET}" if gcp_proj else ""
        print(f"{GREEN}✓ Switched to profile:{RESET} {BOLD}{matched_profile}{RESET} ({CYAN}{info.get('email')}{RESET}){proj_str}")
...
def cmd_status(json_output: bool = False) -> int:
    ...
    active_prof_meta = (PROFILES_DIR / curr / "profile.json") if curr else None
    active_info = extract_account_info(
        ACTIVE_AGY_TOKEN,
        ACTIVE_OAUTH_CREDS,
        ACTIVE_GOOGLE_ACCOUNTS,
        profile_meta_path=active_prof_meta,
    )
    ...
    if not curr and profiles and active_info.get("email") != "Unknown":
        for p in profiles:
            p_info = extract_account_info(
                ...
                profile_meta_path=PROFILES_DIR / p / "profile.json",
            )
    ...
    if json_output:
        ...
        for p in profiles:
            ...
            profile_data.append({
                ...
                "gcp_project": p_info.get("gcp_project"),
                "is_active": (p == curr),
        ...
        out = {
            ...
            "active_gcp_project": active_info.get("gcp_project"),
    ...
    active_proj = active_info.get("gcp_project")
    if active_proj:
        print(f"  {BOLD}GCP Project:{RESET}    {PURPLE}{active_proj}{RESET} {GREEN}(Credits / Vertex AI){RESET}")
    else:
        print(f"  {BOLD}GCP Project:{RESET}    {GRAY}[None linked - Standard Quota]{RESET}")
    ...
    if not profiles:
        ...
        for idx, p in enumerate(profiles, 1):
            ...
            proj_str = f" [{PURPLE}{p_info.get('gcp_project')}{RESET}]" if p_info.get("gcp_project") else f" {GRAY}[No GCP Project]{RESET}"
            print(f"  [{idx}] {marker} {BOLD}{p:<16}{RESET} {CYAN}{p_info.get('email'):<30}{RESET}{proj_str}")
        print()
...
def main() -> int:
    ...
    # set-project / link-project
    p_proj = subparsers.add_parser("set-project", aliases=["project", "link-project"], help="Link a Google Cloud Project ID (with Developer/Cloud Credits) to a profile")
    p_proj.add_argument("project_id", help="GCP Project ID (or 'none'/'clear' to unlink)")
    p_proj.add_argument("target", nargs="?", default=None, help="Target profile name or index (defaults to active profile)")
    ...
    elif args.subcommand in ("set-project", "project", "link-project"):
        return cmd_set_project(project_id=args.project_id, target=args.target)
